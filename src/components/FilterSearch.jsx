...
const FilterSearch = () => {
  ...
  const handleSearch = async (currentPage = 1) => {
    console.log('Searching with:', { searchText, ...filters, page: currentPage });
  ...
  return (
    <div className="">
        ...
        <div className="mt-8 w-full">
          {results.length > 0 ? (
            results.map((result, index) => (
              <LawCard key={index} title={result.title} description={result.description} timePublished={result.timePublished} />
            ))
          ) : (
      ...
      <div className="flex justify-center mt-4 w-full">
          ...
          <span className="px-4 py-2">{page} од {totalPages}</span>
          <button
